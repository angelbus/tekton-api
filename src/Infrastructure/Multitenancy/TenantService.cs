...
using Tekton.API.Application.Common.Exceptions;
...
using Tekton.API.Infrastructure.Persistence.Initialization;
using Mapster;
...
{
    private readonly IMultiTenantStore<TektonTenantInfo> _tenantStore;
    ...
    public TenantService(
        IMultiTenantStore<TektonTenantInfo> tenantStore,
    ...
    public async Task<string> CreateAsync(CreateTenantRequest request, CancellationToken cancellationToken)
    {
        ...
        var tenant = new TektonTenantInfo(request.Id, request.Name, request.ConnectionString, request.AdminEmail, request.Issuer);
    ...
    private async Task<TektonTenantInfo> GetTenantInfoAsync(string id) =>
        await _tenantStore.TryGetAsync(id)
            ?? throw new NotFoundException(_t["{0} {1} Not Found.", typeof(TektonTenantInfo).Name, id]);
}